...
def api_new():
    ...
    extra = {"started_at": started_at, "logged_in": bool(user),
             "in_progress": bool(user)}
...
def api_move():
    ...
    # update the right row. game_id is display/audit; ownership is by user.
    ...
    # Engine reply if game continues.
    last_bot_move = None
    ...
    # Persistence for logged-in users: autosave every move; finalize on end.
    if user:
        if game_over:
            STORE.finish_game(
                user_id=user["id"], game_id=gid, human_color=human_color,
                result=board.result(claim_draw=True),
                result_reason=core.result_reason(board),
                moves_uci=moves_uci, started_at=started_at,
                ended_at=_now_iso())
        else:
...
def api_resign():
    ...
    STORE.finish_game(
        ...
        ended_at=_now_iso())
    return jsonify({"ok": True, "result": result, "result_reason": "resignation"})
